...
export function App(): JSX.Element {
  ...
  const setLifecycle = useCallback(
    async (lifecycle: 'in_work' | 'active' | 'discontinued') => {
      ...
      const pnSeq = currentPn.slice(0, -1)
      try {
        // Gate: every part (designed or purchased) needs a real drawing to
        // go active - auto-generated from a vendor STEP when one is on
        // file, otherwise promotion is refused outright, before anything
        // else here runs (pn.setLifecycle itself does not enforce this,
        ...
        if (lifecycle === 'active') {
          ...
          if (gate.autoGenerated) {
            flashSketchNotice(`${currentPn}: auto-generated a reference drawing from its vendor 3D model.`)
          }
        }
        ...
        if (lifecycle === 'active') {
          ...
          // Promoting an assembly validates everything underneath it too -
          // check first (never promote blind) so a discontinued sub-part
          // gets a real choice instead of being silently resurrected or
          // silently left stuck in_work forever.
          try {
            const check = await api.pnCascadePromoteCheck(currentPn)
            let overrideDiscontinued: string[] | undefined
            if (check.discontinued.length > 0) {
              const list = check.discontinued.map((d) => `  ${d.pn} - ${d.description}`).join('\n')
              const rePromote = window.confirm(
                `${check.discontinued.length} sub-component(s) of ${currentPn} are discontinued:\n\n${list}\n\n` +
                  `OK to re-promote them to active anyway, or Cancel to leave them discontinued ` +
                  `(you'll need to replace them in the assembly separately).`
              )
            ...
            if (check.toPromote.length > 0 || overrideDiscontinued) {
              ...
              if (result.promoted.length > 0) {
                parts.push(
                  `promoted ${result.promoted.length} sub-component(s)` +
                    (autoDrawn > 0 ? ` (${autoDrawn} with an auto-generated drawing)` : '')
                )
              }
              if (result.blocked.length > 0) {
                parts.push(`${result.blocked.length} sub-component(s) still need a drawing before they can be active`)
              }
              if (parts.length > 0) flashSketchNotice(parts.join(' - ') + '.')
              if (result.blocked.length > 0) {
                console.warn('Sub-components blocked from cascade promotion (no drawing):', result.blocked)
              }
            }
          } catch (cascadeErr) {
            console.error('pnCascadePromote:', cascadeErr)
            flashSketchNotice(`${currentPn}: promoted, but sub-component cascade failed - see console.`)
          }
        }
      } catch (e) {
        window.alert((e as Error).message)
