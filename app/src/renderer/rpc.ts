...
export const api = {
  ...
  /** Dry run before pnCascadePromote: walks pn's entire BOM tree (nested
   *  sub-assemblies included) and reports which sub-components aren't
   *  active yet, split into safely-promotable vs. discontinued (a
   *  deliberate state that must never be silently overridden - see
   *  pnCascadePromote's overrideDiscontinued param). */
  pnCascadePromoteCheck: (pn: string) =>
    rpc<{
      pn: string
      toPromote: Array<{ pn: string; pnSeq: string; description: string }>
      discontinued: Array<{ pn: string; pnSeq: string; description: string }>
    }>('pn.cascadePromoteCheck', { pn }),
  /** Promotes pn's entire BOM tree to active. overrideDiscontinued must
   *  list the exact PNs the user explicitly chose to re-promote anyway
   *  (from pnCascadePromoteCheck's discontinued list) - any discontinued
   *  PN not in that list is left alone and reported back in `skipped`.
   *  Each sub-part also goes through the same drawing gate a manual
   *  promotion does (auto-generated from a vendor .stp when possible) - one
   *  that fails it lands in `blocked`, not promoted, not silently skipped. */
  pnCascadePromote: (pn: string, overrideDiscontinued?: string[]) =>
    rpc<{
      pn: string
      promoted: Array<{ pn: string; pnSeq: string; autoGeneratedDrawing: boolean }>
      skipped: Array<{ pn: string; pnSeq: string; reason: string }>
      blocked: Array<{ pn: string; pnSeq: string; reason: string }>
    }>('pn.cascadePromote', { pn, overrideDiscontinued }),
  /** The "must have a drawing to go active" gate - call before pn.setLifecycle
   *  ever sets 'active' (pn.setLifecycle itself does not enforce this).
   *  Auto-generates a reference drawing from a supplier .stp when one is on
   *  file and none exists yet; ok:false means promotion must be refused. */
  pnEnsureDrawingOrBlock: (pn: string) =>
    rpc<{ pn: string; ok: boolean; hadDrawing?: boolean; autoGenerated?: boolean; reason?: string }>(
      'pn.ensureDrawingOrBlock',
      { pn }
    ),
