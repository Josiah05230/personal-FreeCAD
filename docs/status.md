...
- **Scale and Split Face are now real, native, editable PartDesign features**
  ...
  Fillet in the timeline. Both verified genuinely parametric and confirmed to
  survive closing and reopening the file with full edit capability intact.
  Split Face's plane reference stays live (re-derives if the referenced
  geometry changes), not a frozen snapshot.
